...
*/
int PhoneBook::setOption(std::string &choice)
{
    ...
    if (choice == "ADD") _AddNewContact();
    else if (choice == "SEARCH") _SearchContact();
    return (0);
}

void PhoneBook::_AddNewContact(){
    if (_count < 8) {
            _contacts[_count].AddInfo();
            _count++;
        } else {
...
void PhoneBook::_SearchContact(){
	...
	if(_contacts[0].Checkempty()){
		...
		if (_id.length() == 1 && std::isdigit(_id[0]) )
		{
			...
			if (i >= 1 && i <= 8 && !_contacts[i - 1].Checkempty())
			{
